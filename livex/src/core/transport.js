...
export function createTransport({ onState, onBeat } = {}) {
  ...
  function emit() {
    onState && onState({ playing, recording, metronome, bpm, bars, beatsPerBar, events: events.length });
  }
  ...
  function clear() { events = []; openNotes.clear(); emit(); }
  ...
  function recordNoteOn(note, vel) {
    ...
    events.push(ev); openNotes.set(note, ev);
  }
  ...
  function recordPad(index, vel) {
    if (!recording || !playing) return;
    events.push({ type: 'pad', index, vel: vel || 110, pos: posBeats() });
  }

  // one-shot SYNC: quantize every event to the 1/16 grid, then hands off
  function quantize(grid = 0.25) {
    const T = totalBeats();
    for (const e of events) e.pos = (Math.round(e.pos / grid) * grid) % T;
    emit();
  }
  ...
  async function exportWav() {
    ...
    const bd = beatDur();
    const evs = events.map((e) => ({ ...e, pos: e.pos * bd }));
  ...
  return {
    play, stop, toggleRecord, clear, toggleMetronome, setBpm, setBars,
    recordNoteOn, recordNoteOff, recordPad, quantize, exportWav,
    state: () => ({ playing, recording, metronome, bpm, bars, events: events.length }),
    events: () => events,
  };
}
